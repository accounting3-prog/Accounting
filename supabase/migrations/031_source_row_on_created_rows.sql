-- Where a row came from, for rows that do not come from a card sheet.
--
-- Every transaction imported from the workbook carries its source sheet and
-- its row number, and the whole ledger is built on being able to point at the
-- cell a figure came from. Rows written through create_transaction carry
-- neither, because the parameters do not exist — so the 186 bank rows imported
-- through it have no traceable origin at all.
--
-- That is a gap on its own. It also breaks something concrete: created_at
-- defaults to now(), which is fixed for a whole transaction, so every row of a
-- bulk import shares one timestamp and cannot be ordered by it. With no source
-- row either, rows sharing a date have no defined order — and the check that
-- each printed balance matches the running total to that point depends
-- entirely on that order. 284 of 2,431 rows appeared to disagree for no reason
-- other than being compared in an arbitrary sequence.
--
-- Safe to re-run.

begin;

-- The signature as 030 left it.
...
create or replace function create_transaction(
    ...
    p_source_sheet text DEFAULT NULL::text,
    p_source_row integer DEFAULT NULL::integer
)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
...
begin
    ...
    insert into transactions (
        ...
        p_statement_balance, p_source_sheet, p_source_row

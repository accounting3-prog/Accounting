-- The field is called Purchase Order.
--
-- 025 called it po_box, 026 called it po, and this settles it. The name is not
-- only a label: the History screen prints the key of each changed field as it
-- is stored, so the column name is what an auditor reads years from now.
--
-- Checked before renaming, as before: no row carries a value and no edit has
-- ever recorded a change to it, so there is nothing to migrate and no
-- append-only history to contradict. Once someone types a real purchase order
-- into it, this stops being free and the name is settled for good.
--
-- Safe to re-run.
...
do $$
begin
    -- From either of the two earlier names, whichever this database is on.
    ...
    then
        execute 'alter table transactions rename column po to purchase_order';
    elsif exists (select 1 from information_schema.columns
                where table_name = 'transactions' and column_name = 'po_box')
       and not exists (select 1 from information_schema.columns
                where table_name = 'transactions' and column_name = 'purchase_order')
    then
        execute 'alter table transactions rename column po_box to purchase_order';
    end if;
end $$;

alter table transactions add column if not exists purchase_order text;

comment on column transactions.purchase_order is
...
-- ---------------------------------------------------------------------------
-- update_transaction takes p_purchase_order
-- ---------------------------------------------------------------------------
...
-- The 026 signature, whose last parameter is named p_po. A rename is not a
-- replacement: create or replace refuses to rename an existing parameter, and
-- leaving it would put two 21-argument versions side by side.
...
begin
    ...
    -- which the others cannot express: a purchase order is typed by hand, so one that could
    -- be corrected but never erased would be a trap.
    v_purchase_order := case
        when p_purchase_order is null     then old.purchase_order
        when btrim(p_purchase_order) = '' then null
        else btrim(p_purchase_order)
    end;
    ...
    -- Clearing one records a 'to' of null, so removing a purchase order is as traceable as
    -- setting it.
    if old.purchase_order is distinct from v_purchase_order then
        v_changes := v_changes || jsonb_build_object('purchase_order',
            jsonb_build_object('from', old.purchase_order, 'to', v_purchase_order));
    end if;

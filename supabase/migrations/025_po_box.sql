-- A PO Box on a transaction.
--
-- Asked for on the edit screen: a supplier's PO Box, recorded against the
-- charge it belongs to. Free text, because a PO Box is written differently in
-- every country and validating it would only refuse things that are correct.
--
-- It goes through update_transaction like every other field, which means a
-- change to it is recorded with its before and after, its reason and its
-- author. A field that could be changed without that would be the one place in
-- the ledger where something moved and nobody could say who moved it.
--
-- Safe to re-run.

begin;

alter table transactions add column if not exists po_box text;

comment on column transactions.po_box is
    'The supplier''s PO Box for this charge. Free text: postal formats differ by '
    'country and a validator would reject correct ones.';

-- NOT added to search_text, deliberately.
--
-- search_text is a generated column and transactions_searchable is defined as
-- `select t.*`, so its stored column list already fixes search_text's position.
-- Rebuilding the column to widen it would drop the trigram index and break that
-- view, and the app does not read either one: the search runs in the browser
-- over the loaded rows. So the PO Box is made searchable where the search
-- actually happens, in web/src/lib/search.ts, and nothing here is disturbed.

-- ---------------------------------------------------------------------------
-- update_transaction carries it
-- ---------------------------------------------------------------------------

-- Adding a parameter does not replace this function, it overloads it: both the
-- 20-argument and the 21-argument version would exist, and every call naming
-- arguments would then be ambiguous. The old signature goes first.
...
begin
    ...
    -- which the others cannot express: a PO Box is typed by hand, so a typo
    -- that could be changed but never erased would be a trap.
    v_po_box := case
        when p_po_box is null       then old.po_box
        when btrim(p_po_box) = ''   then null
        else btrim(p_po_box)
    end;
    ...
    -- Recorded like any other field: what it was, what it became. Clearing one
    -- records a 'to' of null, so removing a PO Box is as traceable as setting it.
    if old.po_box is distinct from v_po_box then
        v_changes := v_changes || jsonb_build_object('po_box',
            jsonb_build_object('from', old.po_box, 'to', v_po_box));
    end if;

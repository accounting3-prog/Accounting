-- The audited write path for manual entry.
--
-- The browser never inserts into `transactions` directly. It calls this
-- function, which re-applies every rule the importer applies, in the database,
-- where it cannot be bypassed by editing client code or replaying the request
-- with a crafted body:
--
--   * only a named admin may write at all;
--   * the caller supplies a POSITIVE amount and a transaction kind, and the
--     SIGN is derived here — a purchase cannot be recorded as increasing a
--     balance no matter what the client sends;
--   * a reconciliation adjustment is stored with no direction, so it can never
--     land inside a spend or funding total;
--   * choosing a currency requires both the original amount and the rate,
--     unless the row is explicitly marked needs_review;
--   * the dedup key is computed here, including direction and an occurrence
--     number, so a genuine repeat charge gets its own row while a resubmitted
--     form does not create a second one.
--
-- Returns the new transaction id, or the id of the existing row when the exact
-- same submission is replayed.
...
create or replace function create_transaction(
    ...
    p_txn_date          date,
    p_kind              text,
    p_amount_aed        numeric,          -- always positive
    p_supplier          text,
    p_req_number        text,
    p_payment_ref       text,
...
begin
    ...
    -- The sign is decided here, from the kind alone.
    case p_kind
    ...
    -- A converted transaction needs all three parts, or must say it is unfinished.
    ...
    -- Supplier, with the merchant country split out the way the import does.
    v_supplier_raw := btrim(p_supplier) ||
    ...
    -- Content signature, matching the importer's definition exactly.
    ...
    -- The next occurrence for this exact content. A repeat charge gets its own
    -- row; an identical resubmission of a row already stored returns that row.
    ...
    v_dedup := encode(digest(v_signature || '|#' || v_occurrence, 'sha256'), 'hex');
...
comment on function create_transaction is
    'The only write path for manual entry. Derives the sign from the transaction '
    'kind, computes the dedup key with an occurrence number, and refuses any '
    'caller that is not a named admin. The browser never inserts into '
